...
int main(int argc, char *argv[]) {
    ...
    assert(numSingleOccurrences(numSingle_6, 5) == 5);
}
...
*/
int numSingleOccurrences(int arr[], int size) {
    Set seen_once = SetNew();
    Set seen_twice = SetNew();

    for (int i = 0; i < size; i++) {
        if (SetContains(seen_twice, arr[i])) {
            continue;
        } else if (SetContains(seen_once, arr[i])) {
            SetDelete(seen_once, arr[i]);
            SetInsert(seen_twice, arr[i]);
        } else {
            SetInsert(seen_once, arr[i]);
        }
    }

    int num_once = SetSize(seen_once);
    SetFree(seen_once);
    SetFree(seen_twice);
    return num_once;
}